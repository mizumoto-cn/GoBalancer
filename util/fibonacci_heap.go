...
var (
	...
	ErrNegativeInf        = errors.New("negative infinity key is reserved for internal usage ")
	ErrTagNotFound        = errors.New("tag is not found ")
	ErrInvalidKeyTooBig   = errors.New("input key is too big ")
	ErrInvalidKeyTooSmall = errors.New("input key is too small ")
)
...
func (heap *FibHeap) String() string {
	...
	if heap.num != 0 {
		...
		buffer.WriteString(fmt.Sprintf("Current minimum: key(%f), tag(%v), value(%v),\n", heap.min.key, heap.min.tag, heap.min.value))
		buffer.WriteString("Heap detail:\n")
		probeTree(&buffer, heap.roots)
		buffer.WriteString("\n")
	} else {
		buffer.WriteString("Heap is empty.\n")
	}
